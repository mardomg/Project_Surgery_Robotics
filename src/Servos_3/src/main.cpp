...
// 📤 Envia els torques mesurats al Gripper i a l’ordinador
void sendTorquesUDP() {
  // Calcula torques de cada motor
  float Torque_roll1 = getTorque(sumRoll1, PIN_ANALOG_ROLL1, prevRoll1);
  float Torque_roll2 = getTorque(sumRoll2, PIN_ANALOG_ROLL2, prevRoll2);
  float Torque_pitch = getTorque(sumPitch, PIN_ANALOG_PITCH, prevPitch);
  float Torque_yaw   = getTorque(sumYaw, PIN_ANALOG_YAW, prevYaw);

  // Crea JSON amb els valors
  JsonDocument doc;
  doc["device"] = deviceId;
  ...
  // Envia al Gripper
  udp.beginPacket(IPAddress(192,168,1,11), udpPort);  // 🔹 IP del Gripper
  udp.write((const uint8_t*)jsonBuffer, strlen(jsonBuffer));
  udp.endPacket();

  // Envia també a l'ordinador (opcional, per monitoritzar)
  udp.beginPacket(IPAddress(192,168,1,15), udpPort);
  udp.write((const uint8_t*)jsonBuffer, strlen(jsonBuffer));
  udp.endPacket();

  Serial.println("Torques enviats via UDP.");
}
